...
/*   Updated: 2025/08/06 16:57:25 by jmeouchy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_file_executable(t_envp *env, t_tree_node *node,
...
void	parsing_main(t_envp *env, char *input, t_gc_list *grbg_collector)
{
	...
	if (tree->root->token >= WORD)
	{
		if (check_file_executable(env, tree->root, grbg_collector) == -1)
			env->exit_code = print_message_and_exit(tree->root->data,
