...
/*   Updated: 2025/07/16 20:48:06 by jmeouchy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *quoted_section(char *input, int *i, int *segment_start, char *quote, t_gc_list *grbg_collector)
{
	char	*temp;
	...
	*segment_start = *i;
	while (input[*i] && input[*i] != *quote)
		(*i)++;
	temp = ft_substr(input, *segment_start - 1, (*i - *segment_start) + 2, grbg_collector);
	if (input[*i] == *quote)
		(*i)++;
	return (temp);
}
char *unquoted_section(char *input, int *i, int *segment_start, t_gc_list *grbg_collector)
{
	char	*temp;
	...
	*segment_start = *i;
	while (input[*i] && input[*i] != ' ' && input[*i] != '|'
		...
		&& input[*i] != '"' && input[*i] != '\'')
		(*i)++;
	temp = ft_substr(input, *segment_start, *i - *segment_start, grbg_collector);
	return (temp);
}
